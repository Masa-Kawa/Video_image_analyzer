...
動画の全フレーム（サンプリングレートに基づく）における赤色率と変化量を記録したファイル。
Step 1 の出力であり、Step 2 の入力となる。
...
| 列名 | 型 | 説明 |
|------|----|------|
| `t_sec` | float | 動画先頭からの経過秒数（小数点以下3桁） |
| `t_srt` | string | SRT形式のタイムスタンプ (`HH:MM:SS,mmm`) |
| `red_ratio` | float | 計算された赤色率（0.0〜1.0） |
| `delta` | float | 前フレームからの赤色率増分 |
| `smooth_delta` | float | deltaの移動平均（平滑化後の値）。イベント抽出に使用される。 |
| `reader` | string | フレーム読み込みに使用したバックエンド (`pyav` or `opencv`) |

---

## 2. SRT フォーマット

### 2a. 指標SRT (`{video_name}_metrics.srt`)

CSVの数値を動画上に表示するための字幕ファイル。`src.tools.csv_to_srt` で生成する。
...
red=0.0156 Δs=0.0017
```

- **表示項目**: デフォルトでは `red_ratio` (red) と `smooth_delta` (Δs)。

### 2b. イベントSRT (`{video_name}_bleed.srt`)

検出された出血候補イベント、または人手で作成したイベントを示す字幕ファイル。
JSONL正本から生成されるほか、Shotcut等で編集してJSONLに書き戻すことができる。
...
- **JSON行**: イベントのメタデータを含むJSON文字列。**時刻情報はSRTのタイムスタンプが優先されるため、JSON内には含めない（または無視される）。**

---

## 3. JSONL フォーマット（イベント正本）

イベントデータの正本（Single Source of Truth）。
1行に1つのイベントオブジェクトをJSON形式で記述する。
