...
- **ディレクトリ地図・使い方ガイドを新規作成**: `docs/architecture.md`（3層〈入口/解析エンジン/内部部品〉での整理、人が直接動かすプログラムの一覧、やりたいこと別早見表、入口の見分け方）。`README.md` 構成節からリンク。
- **TUTORIAL.md の Step 1 に「複数動画をまとめてプロキシ作成」を追記**（ファイル列挙／ワイルドカード／`--series` の3方法、通常と `--series` の違い、`videos`・`--series` のオプション説明）。
